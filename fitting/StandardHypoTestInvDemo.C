...
RooStats::HypoTestInvTool::AnalyzeResult( HypoTestInverterResult * r,
   ...
   std::string fname = "20140423_CFscan_WithSyst/";
   ...
   if (r != NULL && mWriteResult) {
      ...
      if (mResultFileName.IsNull()) {
         mResultFileName = TString::Format("20140423_CFscan_WithSyst/%s_%s_%s_ts%d_",calcType,limitType,scanType,testStatType);      
   ...
   TCanvas * c1 = new TCanvas("c1",c1Name); 
   c1->SetLogy(false);
   ...
   if (mPlotHypoTestResult) { 
     ...
     if (r != NULL && mWriteResult) {
       std::string f1name = fname;
       std::string f2name = fname;
       std::string ext = "Workspace.root";
       f1name.replace(f1name.find(ext),ext.length(),"CFscan.pdf");
       f2name.replace(f2name.find(ext),ext.length(),"ToyMC.pdf");

       c1->SaveAs(f1name.c_str());
       c2->SaveAs(f2name.c_str());
     }
   }
