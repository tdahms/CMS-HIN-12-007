...
void fitMC_2CB(bool isHI=false, double ptmin=0.0, double ptmax=30.0, double ymin=0.0, double ymax=2.4, bool absRapidity=true, bool fitChi2=false, bool savePlot=false)
{
  ...
  RooRealVar wideFactor("n_{CB2}","n_{CB2}",2.0,1.0,6.0);
  RooFormulaVar sigma2("#sigma_{CB2}","@0*@1",RooArgList(sigma1,wideFactor));
  RooCBShape cball2("cball2","cball2",Jpsi_Mass,mean,sigma2,alpha,n);

  RooRealVar coeffGauss("f_{CB2}","f_{CB2}",0.1,0.0,1.0);
  RooAddPdf cbg("cbg","cbg",RooArgList(cball2,cball),coeffGauss);
  ...
  model_cbg.plotOn(xframe, Components(RooArgSet(cball2)), LineColor(kGreen+2), LineStyle(kDashed));
  ...
  std::cout << "CB + CB2:\t" << *CBGalpha << "\t" << *CBGn << std::endl;
  ...
  if (fitChi2) {
    if (isHI)
      outfname = Form("20140411_MC_plots/Jpsi_PbPb_MCshape_Rap_%3.1f-%3.1f_Pt_%3.1f-%3.1f_M2234_chi2.pdf",ymin,ymax,ptmin,ptmax);
    else
      outfname = Form("20140411_MC_plots/Jpsi_pp_MCshape_Rap_%3.1f-%3.1f_Pt_%3.1f-%3.1f_M2234_chi2.pdf",ymin,ymax,ptmin,ptmax);
  }
  else {
    if (isHI)
